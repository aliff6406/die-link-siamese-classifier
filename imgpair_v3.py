...
class SiamesePairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        unique_labels = self.image_df[1].unique()
        chosen_label = random.choice(unique_labels)
        
        # Get indices of all images within the chosen class
        class_indices = self.image_df[self.image_df[1] == chosen_label].index.tolist()
        ...
        if positive_pair:
            # Ensure there is more than one image in this class
            if len(class_indices) == 1: # Handles edge case if there is only one image in this class
                pair_idx = idx # Use the same image to pair to itself
            else:
                class_indices.remove(idx) # Remove the current image's index so that it isn't paired with itself
                pair_idx = random.choice(class_indices)
        else:
            # Choose a different class
            different_label = random.choice([label_ for label_ in unique_labels if label_ != chosen_label])
